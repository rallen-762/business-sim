...
4. Teacher dashboard segment-breakdown tab — revenue/units by segment, agreed conceptually, not yet designed in detail. *(Superseded — screen-map.md Section 4 now specifies this in full as a bar chart. Treat the screen-map version as current; this note is stale but left here for history.)*
...
No code, database schema, or architecture has been built yet — this document is the economic-model spec only. *(Historical note: this line was true when the doc was originally written. As of this session, the full app has been built — see the main repo and docs/screen-map.md's "Known conflicts" section for where the build diverges from this spec.)*
