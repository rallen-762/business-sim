...
17. Only 2 of the original 4 reference docs are saved in this repo (this file and master-variable-table.md). `teacher-reference-guide.txt` and `demand-formula-visual.txt` were also part of the original spec but were never saved to disk and their literal text was lost to an earlier context compaction -- re-paste them if/when available so they can be added here too.
