...
17. `teacher-reference-guide.txt` and `demand-formula-visual.txt` (both now saved, 2026-09-08) checked line-by-line against `constants.py`/`engine.py` -- every number matches exactly (ad ladder, demand formula, all 4 segment multiplier tables, the Wealthy piecewise formula). Zero new conflicts from either. All 4 of the original reference docs are now saved under `docs/`.
