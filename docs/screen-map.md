...
15. master-variable-table.md's "PINNED FOR LATER" item 4 ("Teacher dashboard segment-breakdown tab... not yet designed in detail") is stale -- this screen-map's Section 4 now specifies it in full as a bar chart. Treat this screen-map as current for that item; the master doc's note is a leftover from before that section was written.
16. master-variable-table.md's closing line ("No code, database schema, or architecture has been built yet") is also stale -- true when that doc was originally written, no longer true as of this session. Left in that doc verbatim (it's the source-of-truth doc, not something to silently edit) but flagged here so nobody reads it as current.
17. `teacher-reference-guide.txt` (now saved, 2026-09-08) checked line-by-line against `constants.py`/`engine.py` -- every number matches exactly (ad ladder, demand formula, all 4 segment multiplier tables, the Wealthy piecewise formula). Zero new conflicts. `demand-formula-visual.txt` is still missing (lost to the same earlier compaction; re-paste if available).
18. `teacher-reference-guide.txt`'s "OPEN / STILL PENDING" section (its Section 5) is stale on two of its four points: unsold-inventory handling and the Pricing Guidance Tool's Round 1 fallback are both listed there as undesigned, but `master-variable-table.md` (Section 7b and Section 13, respectively) has since locked both. Treat the master doc as current for those two; this guide's other two pending items (student user guide, Teacher Dashboard segment tab) are still genuinely open, matching items 9 and the screen-map's own scope above.
